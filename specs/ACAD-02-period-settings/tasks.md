...
- [x] `institution.model.ts` — subdocumento `settings` (`periodsPerYear` default 4, `enabledReports` default ambos, `{ _id: false }`, `default: () => ({})`).
...
- [x] `period.service.ts` — tope por `settings.periodsPerYear` para el `year` → `AppError(409)`.
...
- [x] `features/period/components/PeriodsPanel.tsx` — `DataTable` + `FormModal` + `ConfirmationModal`; marca del periodo activo.
...
- [x] `features/institution/components/ReportSettingsPanel.tsx` — checkboxes + input `periodsPerYear` (mismo formulario, un solo PATCH); submit deshabilitado con ambos desmarcados o sin cambios.
...
- [x] Exceder `periodsPerYear` → `409`; fechas invertidas → `400`
