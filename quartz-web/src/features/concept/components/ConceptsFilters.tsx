...
import { QualitativeValuation } from "../types";
...
interface ConceptsFiltersProps {
    periods: FilterOption[];
    subjects: FilterOption[];
    selectedPeriods: string[];
    selectedSubjects: string[];
    selectedValuationTypes: QualitativeValuation[];
    onTogglePeriod: (periodId: string) => void;
    onToggleSubject: (subjectId: string) => void;
    onToggleValuationType: (valuationType: QualitativeValuation) => void;
}

export function ConceptsFilters({
    periods,
    subjects,
    selectedPeriods,
    selectedSubjects,
    ...
    onTogglePeriod,
    onToggleSubject,
    onToggleValuationType,
}: ConceptsFiltersProps) {

    const hasFilters = selectedPeriods.length > 0 || selectedSubjects.length > 0 || selectedValuationTypes.length > 0;
            ...
            <div className="flex gap-4 items-center border border-gray-200 bg-white rounded-xl p-2 px-4 w-fit max-w-full flex-wrap">
                ...
                <Menu dismiss={{ itemPress: false }}>
                    ...
                    <MenuList className="max-h-72">
                        {VALUATION_TYPES.map((type) => (
                            <MenuItem key={type} className="p-0">
                                <label
                                    htmlFor={`filter-valuation-${type}`}
                                    className="flex cursor-pointer items-center gap-2 p-2 hover:bg-gray-100 w-full"
                                >
                                    <Checkbox
                                        crossOrigin={undefined}
                                        id={`filter-valuation-${type}`}
                                        ripple={false}
                                        className="hover:before:opacity-0"
                                        containerProps={{ className: "p-0" }}
                                        checked={selectedValuationTypes.includes(type)}
                                        onChange={() => onToggleValuationType(type)}
                                    />
                                    <Typography color="blue-gray" className="font-normal">
                                        {type}
                                    </Typography>
                ...
                {hasFilters && (
                    <div className="flex flex-wrap gap-2 items-center pl-4">
                        ...
                        {selectedValuationTypes.map((type) => (
                            <Chip
                                key={type}
                                value={type}
                                onClose={() => onToggleValuationType(type)}
                                variant="ghost"
                                color="purple"
                                size="sm"
                                className="rounded-full"
